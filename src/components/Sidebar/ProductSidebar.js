    ...
    <>
      {/* <nav className="md:left-0 md:block md:fixed md:top-0 md:bottom-0 md:overflow-y-auto md:flex-row md:flex-nowrap md:overflow-hidden shadow-xl bg-blueGray-200 flex flex-wrap items-center justify-between relative md:w-64 z-10 py-4 px-6"> */}
        <div className="md:flex-col md:items-stretch md:min-h-full md:flex-nowrap px-0 flex flex-wrap items-center justify-between w-full mx-auto">
          ...
          <div
            className={
              "md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:mt-4 md:shadow-none shadow relative top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded " +
              collapseShow
            }
          >
            {/* Collapse header */}
            ...
            <ul className="md:flex-col md:min-w-full flex flex-col list-circle">
              {menuItems.map((menu, index) =>{
                if(menu.catType === "sub"){
                  return(
                      <li key={index} className="items-center" style={{marginLeft:35}} onClick={()=>setCollapseShow("hidden")}>
                        <a
                          className={
